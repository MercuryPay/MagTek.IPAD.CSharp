...
namespace IPADTest
{
    ...
    {
        public static XDocument BuildXMLRequest(this Dictionary<string, object> dictionary)
        {
            var tStream = new Dictionary<string, object>();
            tStream.Add(GetSecondElement(dictionary), dictionary);
        ...
        private static string GetSecondElement(Dictionary<string, object> dictionary)
        {
            string secondElement = "Transaction";

            if (dictionary.ContainsKey("TranCode")
                && dictionary["TranCode"] is string)
            {
                string tranCode = dictionary["TranCode"].ToString();

                switch (tranCode.ToLower())
                {
                    case "batchsummary":
                    case "itemdetail":
                    case "batchclear":
